...
Query(SpreeInventoryUnit)
.where("id = ?")
# Q 35 : # OptionType.order(:name)
Query(SpreeOptionType)
.order('name')
...
Query(SpreeReturnItem)
.where("exchange_inventory_units = ?")
...
Query(SpreeStockLocation)
.joins('stock_items')
.where("variant_id = ?")
.distinct('')
# Q 96 : # Spree::StoreCreditCategory.order(:name)
Query(SpreeStoreCreditCategory)
.order('name')
# Q 97 : # Spree::TaxCategory.order(:name)
Query(SpreeTaxCategory)
.order('name')
...
Query(SpreeStoreCredit)
.includes("credit_type")
.order('credit_type.priority')
...
.includes('shipment')
.includes('order')
.where("shipment.state != ?")
.where("variant_id = ?")
.where("order.completed_at = ?")
...
Query(SpreeVariant)
.joins('stock_items')
.where("stock_items.backorderable = ?")
...
Query(SpreeReturnAuthorization)
.where("order_id = ?")
.where("id = ?")
...
Query(SpreeReturnItem)
.includes('exchange_inventory_units')
.where('exchange_inventory_units.original_return_item_id = NULL')
.distinct('')
# Q 250 : # return_items.select(&:exchange_required?)
Query(SpreeReturnItem).includes('exchange_inventory_units')
.where('exchange_inventory_units.original_return_item_id = NULL')
.distinct('')
...
# kind.classify}")
Query(SpreeZoneMember)
.where('zoneable_id != NULL')
.pred_or("zoneable_type != ?")
...
Query(SpreeIncompleteReimbursementError)
