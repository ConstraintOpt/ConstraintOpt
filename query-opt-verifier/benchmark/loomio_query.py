...
.where("eventable_id = ?")
.select("sequence_id")
# Q 30 : # Event.where(id: @events.map(&:id)).includes(:sequence_id)
Query(Event)
.select("sequence_id")
.where("id = ?")
# Q 31 : # Event.where(id: discussion.forked_event_ids).pluck(:discussion_id).unique.length
Query(Event)
.where("id = ?")
...
.return_limit('1')
# # Q 39 : # Group.published.where(id: ids)
# #          .or(Group.published.where("parent_id in (:ids)", ids: ids)
# #                             .where("is_visible_to_parent_members = true or is_visible_to_public = true")
# #              )
# Query(Group)
# .where("id = ?")
# .pred_or("(parent_id in (?,?)) || (is_visible_to_parent_members = ?) || (is_visible_to_public = ?)")
...
Query(Group)
.where("id = ?")
.select("parent_members_can_see_discussions")
...
Query(Group)
.where("id = ?")
.select("members_can_create_subgroups")
.return_limit("1")
# # Q 164 : # group_ids = (
...
.where("stance_choices.poll_option_id = ?")
.distinct()
...
Query(Group)
.where("id = ?")
.select("is_visible_to_parent_members")
# Q 219 : # tag.group.is_visible_to_public?
Query(Group)
.where("id = ?")
.select("is_visible_to_public")
...
Query(EventsGroupIdentityCreated)
.where('user_id = ?')
.order('created_at')
...
Query(Attachment)
.where("migrated_to_document = false")
