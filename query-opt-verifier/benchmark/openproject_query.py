...
Query(QueriesOperatorsOrderedWorkPackages)
...
.joins('work_packages')
.joins("repository")
.select('wiki.filesize')
.select('repository.required_storage_bytes')
# .select('repos.required_storage_bytes')
...
Query(LdapGroupsSynchronizedGroup)
.where("id = ?")
...
Query(TwoFactorAuthenticationDevice)
.where("default = ?")
.where("active = ?")
...
.where('attachments.container_id != NULL')
...
Query(ActivitiesDocumentActivityProvider)
.where("id = ?")
...
Query(JournalAggregatedJournal)
.order('id')
.return_limit('1')
