...
class QueryField(object):
  ...
  def f(self, field_name, table=None): #one to many relation
    new_qf = QueryField(field_name, table=self.field_class)
  ...
  def get_necessary_index_keys(self):
    if isinstance(f.field_class, Table):
      return []
    else:
      return [self]
  def get_all_fields(self):
...
class AssocOp(Pred):
  ...
  def f(self, field_name):
    ...
    newf = QueryField(field_name, table=last.rh.field_class)
  ...
  def get_all_fields(self):
    return [self.lh] + self.rh.get_all_fields()
  def get_necessary_index_params(self):
    return []
  def complete_field(self, table):
...
"""
def parse_pred(table, str):
  # TODO: fix grammar
  if 'exists(' in str:
    index = str.find('.exists(')
    assoc = parse_pred_atom(table, str[:index])
    assert(isinstance(assoc, QueryField) or isinstance(assoc, AssocOp))
    next_table = assoc.field_class if isinstance(assoc, QueryField) else assoc.lft.field_class
    idx = str.replace('exists(','')[:-1]
    pred = parse_pred(next_table, str[index+len('.exists('):-1])
    return SetOp(assoc, EXIST, pred)
  elif str.startswith('!'):
    return UnaryOp(parse_pred(table,str[1:]))
  else:
    if ' AND ' in str:
      chs = str.split(' AND ')
      return ConnectOp(parse_pred(table, chs[0]), AND, parse_pred(table, chs[1]))
    elif ' OR ' in str:
      chs = str.split(' OR ')
      return ConnectOp(parse_pred(table, chs[0]), OR, parse_pred(table, chs[1]))
    else:
      chs = str.split(' ')
      lft = parse_pred_atom(table, chs[0])
      op = sql_op_to_pred_map[chs[1]]
      rgt = parse_pred_atom(table, chs[2])
      return BinOp(lft, op, rgt)
      
def parse_pred_atom(table, atom):
  if atom == '?':
    rt = Parameter('param-{}'.format(globalv.param_counter))
    globalv.param_counter += 1
  elif atom.startswith('param['):
    rt = Parameter('{}'.format(atom[:-1].replace('param[','')))
  elif '.' in atom:
    chs = atom.split('.')
    tables = [table]
    for c in chs[:-1]:
      tables.append(get_main_table(tables[-1].get_nested_table_by_name(c)))
    rt = None
    for i in reversed(range(len(chs))):
      rt = f(chs[i],tables[i]) if rt is None else AssocOp(QueryField(chs[i],tables[i]), rt)
  elif table.get_assoc_by_name(atom):
    rt = QueryField(atom, table)
  elif table.get_field_by_name(atom):
    rt = QueryField(atom, table)
  else:
    if atom.startswith("'"):
      rt = AtomValue(atom, 'string')
    elif atom in ["true","false"]:
      rt = AtomValue(atom=='true', 'bool')
    else:
      rt = AtomValue(int(atom), 'int')
  return rt
"""