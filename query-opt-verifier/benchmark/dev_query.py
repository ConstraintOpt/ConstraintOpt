...
.where('social_preview_template != ?')
.where('social_preview_template != NULL')
.return_limit('1')
...
Query(Article)
.where('nth_published_by_author > 0')
.where('nth_published_by_author < 4')
.where('published_at > ?')
...
# .includes('user')
# .where("id = ?")
# # Q 72 : # Article.published.incldes(user: [:notes]).limited_columns_internal_select.order("hotness_score DESC").page(params[:page]).per(30)
# Query(Article)
# .select('published')
# .includes('user')
# .order('hotness_score')
...
# .select('published')
# .where("featured = ?")
# .where("featured_number > ?")
# .includes('user')
# .includes('buffer_updates')
# .order('featured_number')
...
.where("(has_unopened_messages = ?) || (status = ?)")
...
Query(User)
.where("(old_username = ?) || (old_old_username = ?)")
.return_limit("1")
...
# Query(User)
# .where("(github_created_at > ?) || (twitter_created_at > ?)")
# .where("created_at > ?")
# .order('created_at')
# .return_limit('150')
