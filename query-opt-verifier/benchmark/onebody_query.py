...
Query(Person).joins('memberships').where('memberships.group_id in (?, ?)').where('id != ?')
...
# .where("person_id = ?")
# .order('created_at')
# Q44 owner is `belongs_to :owner, polymorphic: true`
...
Query(Family)
.includes('people')
.order('last_name, name, people.position')
...
.where('deleted = false')
.joins('family')
.where('(family.barcode_id = ?) || (family.alternate_barcode_id = ?)')
...
Query(Admin)
.includes('people')
.where("people.memberships.group_id = ?")
...
.where('group_times.checkin_time_id = NULL')
.order('group_times.sequence')
# Q 328 : # joins("left join document_folder_groups dfg on dfg.document_folder_id = document_folders.id").where("dfg.id is not null")
Query(DocumentFolder)
.joins('document_folder_groups')
.where('document_folder_groups.document_folder_id = id')
.where('document_folder_groups.id != NULL')
...
Query(Person)
.includes('friendship_requests')
.where("friendship_requests.from_id = ?")
.where("friendship_requests.rejected = ?")
...
.includes('family')
.where('business_name = ?')
#Person.eager_load(:family, :groups)
...
.includes('family')
.includes('groups')
.where("groups.category != ?")
.pred_or("groups.category = NULL")
...
.includes('family')
.includes('groups')
.where("family.deleted = ?")
# Q 379 : # where!(families: { visible: true })
Query(Person)
.includes('family')
.includes('groups')
.where("family.visible = ?")
...
Query(CheckinTime)
.where("(the_datetime >= ?) && (the_datetime <= ?)")
.pred_or("weekday = ?")
...
Query(Group)
.where("link_code != NULL")
.where("link_code != ''")
# Q 396 : # where("parents_of is not null")
Query(Group)
.where("parents_of != NULL")
