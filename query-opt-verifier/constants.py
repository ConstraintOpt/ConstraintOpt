...
sql_op_to_pred_map = {v:k for k,v in pred_op_to_sql_map.items()}
sql_op_to_pred_map['<>'] = NEQ
...
def schema_type_to_type(stype):
  if stype in ['boolean','binary']:
    return 'bool'
  if stype == 'integer':
    return 'int'
  if stype == 'text':
    return 'string'
  if stype in ['time','datetime','timestamp']:
    return 'date'
  return stype
